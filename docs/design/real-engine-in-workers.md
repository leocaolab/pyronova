...
| FR-17 | Setters that write **process-global** config apply only on main (review N13): `set_max_body_size` (→ `handlers.rs:144` `MAX_BODY_SIZE`) and `configure_compression` (→ `compression.rs:55-60`). A worker's script calls them again while it executes; there the call is a no-op, and if the value differs from the current global it logs a warning naming the setter and both values, instead of racing main's value. (`METRICS_ENABLED`, `monitor.rs:62`, is written only by `run()`, which returns early in a worker.) **Superseded:** limits and compression are per app in `SiteConfig` (`src/site.rs`; decision G1 in `docs/design/code-review-final-rubric.md`); a worker's call sets its own app's value, which is never served, and a limit that differs from main's is logged as ignored. | F1 |
...
| FR-20 | The worker executes the script as a **real module** (M4 review N9): the bootstrap runs first in its own namespace; the user script is compiled with `compile(src, script_path, "exec")` into `types.ModuleType("__pyronova_worker__")` registered in `sys.modules` before exec. Today bootstrap + script are one `PyRun_String` (`worker.rs:123-124,227`): `from __future__ import annotations` in a user script is a SyntaxError in every worker (measured), tracebacks show `<string>` with offset lines, and `typing.get_type_hints` on a worker dataclass raises (no `sys.modules[__name__]`). `_async_engine.py` also runs in its own namespace, not the user's globals (N1d). **The script's imports resolve as on main** (review cced8c2, M5 follow-up): before the bootstrap runs, the worker's `sys.path` is set to main's `sys.path` as it is when the workers are built (`WorkerSpec::import_path`). A new interpreter's own `sys.path` holds only what the process started with, so the entries main added at run time were missing: `pyronova run`'s working directory (`cli.py`), a test runner's root, the program's own inserts — an app importing a module next to it failed in every worker under the console script. | F1, F7 |
...
| app.rs / pool.rs → worker.rs | C3 | `InterpreterPool::new(split, py, &WorkerSpec)`, `SubInterpreterWorker::new(worker_id, &WorkerSpec)`, `SubInterpreterWorker::end(self)`; `WorkerSpec { program: &WorkerProgram { script_path, script, import_path }, expected: &RouteSignature, pool_id, shared_state, gc_threshold, limits }`, one `PyronovaApp::worker_program` for the TPC and pool builders | name lists replaced by a plain signature (M4 N3) + worker index; explicit teardown (FR-19); main's `sys.path` (FR-20) |
...
main (PyronovaApp.run, main interpreter, GIL held):
 ...
 4. sys.path = main's sys.path at step 3 (FR-20); exec the slim bootstrap in its own namespace (__pyronova_bootstrap__, WORKER_ID=i)
...
- `app.max_body_size = …` / `app.enable_compression(…)` executed in a worker's replay are
  ...
  warning (FR-17).
  *Superseded by per-app settings (`src/site.rs`): see FR-17.*
