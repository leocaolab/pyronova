...
from pyronova.engine import Mode, PyronovaApp as _PyronovaApp, Response, SharedState, init_logger, emit_python_log, _in_worker, _route_params
...
class Pyronova:
    ...
    def _serve(self, settings: _ServeSettings, start: Callable[[_ServeSettings], None]) -> None:
        ...
        abandoned = ", ".join(map(str, self._engine._take_abandoned_workers()))
        if abandoned:
            _logging.getLogger("pyronova.app").error(
                "exiting without finalization: worker(s) %s did not stop within the "
                "shutdown grace period", abandoned
            )
            print(
                "pyronova: worker(s) " + abandoned + " did not stop within the "
