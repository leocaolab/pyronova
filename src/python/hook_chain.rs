...
use super::request_context::{RequestContext, Returned};
use crate::handlers::error::{HandlerError, Stage};
...
/// `Stream` is an error (streaming needs `gil=True, stream=True`, FR-16).
...
impl<'py, L: EventLoop> Chain<'_, 'py, L> {
    ...
    fn call<A: PyCallArgs<'py>>(
        ...
        match Returned::of(&value) {
            Returned::Value => Ok(value),
            // `async def`: a task in the request's own context, so its `ContextVar` writes
            // reach the rest of the request.
            Returned::Coroutine => {
                let event_loop = self.event_loop.event_loop(py)?;
                self.rc.run_coroutine(&event_loop, &value).map_err(raised)
            }
            // Any other awaitable carries its own context.
