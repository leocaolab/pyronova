...
use crate::error::{
    panic_message, refuse, HandlerError, Refusal, RequestLabel, ResponseError, Stage,
};
...
impl Pending {
    ...
    fn answer(self, result: Result<ResponseData, HandlerError>) {
        let reply = result.map_err(|e| e.log(&self.label.tag()));
        if self.reply.send(reply).is_err() {
            tracing::debug!(target: "pyronova::server", request_id = %self.label.id, "the caller timed out (504); dropping the result");
        }
        WorkRequest::inc_completed();
    }
}
...
impl Drop for AsyncJob {
    fn drop(&mut self) {
        if let Some(pending) = self.reply.get_mut().take() {
            pending.answer(Err(refuse(Refusal::WorkerLost(
...
pub(crate) fn _worker_timed_out(job: &Bound<'_, AsyncJob>) -> PyResult<()> {
    no_panic("_worker_timed_out", || {
        job.get().answer(Err(refuse(Refusal::Timeout)))
    })
}
