...
if __name__ == "__main__":
    app.run(host="127.0.0.1", port={PORT}, mode="subinterp", workers=2)
'''
...
def test_async_workers_outlive_30s_and_stop_cleanly(tmp_path):
    ...
    try:
        ...
        result = {}
        t = threading.Thread(target=lambda: result.update(status=_try("/slow", 20)))
        t.start()
        t.join(30)
        assert result.get("status") == 200, (result, log.read_text()[-3000:])
        assert [_get("/a") for _ in range(20)] == [200] * 20
